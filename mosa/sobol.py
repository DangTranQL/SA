...
class custom_sobol():
    ...
    def _evaluate_batch(self, X):
        ...
        F = []
        for b in batch:
            f = self.objectives([b[p] for p in self.param_names], self.choice1, self.choice2)  # expects dict
    ...
    # =========================
    def run_adaptive(self, runs=3, batch_size=1000, explore_frac=0.5, use_tqdm=False, refine_step=0.05):
        """
    ...
    def plot(self, time):
        ...
        if self.circuit == 'posneg':
            plt.savefig(f'out/sobol_{self.circuit}_{self.choice1}{self.choice2}.jpg')
        else:
            plt.savefig(f'out/sobol_{self.circuit}.jpg')
